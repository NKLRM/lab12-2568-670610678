...
export default function HomePage() {
  ...
  const toggleDoneTask = (taskId: string) => {
    setTasks((prev) =>
      prev.map((t) =>
        t.id === taskId ? { ...t, isDone: !t.isDone, doneTime: new Date() } : t
      )
    );
  };
            ...
            <Card withBorder shadow="sm" radius="md" mb="sm" key={task.id}>
              <Group justify="space-between" align="flex-start">
                <Stack>
                  ...
                  {/* แสดง Date & Time */}
                  {task.isDone && task.doneTime && (
                    <Text size="xs" c="kunakorn">
                      Done at: {task.doneTime.toLocaleDateString()},{" "}
                      {task.doneTime.toLocaleTimeString()}
                    </Text>
                  )}
                </Stack>
